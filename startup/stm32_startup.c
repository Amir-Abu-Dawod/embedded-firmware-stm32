...
void Reset_Handler(void)
{
	...
	uint32_t size = (uint32_t)&_edata - (uint32_t)&_sdata; //edata and sdata are addresses only not values
	...
	//call init function of std. library (only if we use this library in our code)
	__libc_init_array();
